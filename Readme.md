...
2. **MessagingComponent** (компонент обмена сообщениями)
    * Базовый абстрактный класс для продюсера и потребителей.
    * Содержит логгер и константы общих настроек кафки (`BOOTSTRAP_SERVERS`, `SCHEMA_REGISTRY`, `TOPIC`, `SENDING_PERIOD_MS`).

3. **AvroProducer** (продюсер) — **2 реплики**
...
4. **AvroConsumer** (потребитель)
    * Базовый абстрактный класс для потребителей.
    * Содержит константы общих настроек кафки (`REQUEST_TIMEOUT_FETCH_MAX_WAIT_DELTA_MS`, `GRACEFUL_SHUTDOWN_WAIT_MS`).
    * Содержит метод получения настроек кафки с учетом переданных параметров (`groupId`, `enableAutoCommit`, `minimumFetchInBytes`, `maximumFetchWaitInMillis`).
    * Содержит метод обработки завершения приложения (`graceful shutdown`).
    * Содержит метод обработки сообщения (имитация бизнес-логики).

5. **AvroSingleConsumer** (одиночный потребитель) — **2 реплики**
...
6. **AvroBatchConsumer** (пакетный потребитель) — **2 реплики**
    * Обрабатывает пакет сообщений при достижении брокерами заданного суммарного лимита сообщений (10) или  по истечении тайм-аута.
...
1. **Общие**
    ...
    * `SENDING_PERIOD_MS` — периодичность отправки сообщений продюсером в мс.

2. **Продюсер**
    * `RETRY_BACKOFF_MS` — пауза в мс между попытками переотправки сообщения при ошибке.

3. **Потребители**
    * `GROUP_ID` — идентификатор группы потребителей.
    * `REQUEST_TIMEOUT_FETCH_MAX_WAIT_DELTA_MS` — разница между максимальным временем накопления данных брокером в топике и тайм-аутом сетевого запроса.
    * `GRACEFUL_SHUTDOWN_WAIT_MS` — время ожидания закрытия потребителя при выходе из приложения.
    * `FETCH_MIN_BYTES` — минимальное количество данных в топике в байтах, которое брокер должен накопить перед ответом потребителю.

3. **Одиночный потребитель**
    * `FETCH_MAX_WAIT_MS` — максимальное время накопления данных брокером в топике перед ответом потребителю.

3. **Пакетный потребитель**
    * `BATCH_SIZE` — количество сообщений в одном пакете (батче).
